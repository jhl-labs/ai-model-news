---
model_id: "Qwen/Qwen-Image-2.1-PE-T2I"
title: "Qwen Image 2.1 PE T2I"
org: "Qwen"
task: "text-to-image"
license: "other"
params: "9.4B"
likes: 35
downloads: 876
discovered_at: "2026-09-21"
created_at: "2026-09-20"
hf_url: "https://huggingface.co/Qwen/Qwen-Image-2.1-PE-T2I"
tags: ["safetensors", "qwen3_5", "qwen", "prompt-rewriting", "text-to-image", "license:other", "region:us"]
reason: "new, updated, major-org"
---

## 왜 주목받는가

최근 60일 내 최초 공개된 신규 모델입니다. 최근 14일 내 의미 있는 갱신이 있었습니다. 주요 기관이 최근 30일 안에 공개한 신작입니다. 이 기준에 해당해 선정했습니다. 좋아요 35개, 다운로드 876회(수집 시점 2026-09-21). 이전 모델 Qwen/Qwen-Image-2.1(파라미터 7.1B, 다운로드 183) 대비 이번 모델은 파라미터 32.4% 증가, 다운로드 378.7% 증가, 라이선스 동일(other).

## 핵심 스펙

| 항목 | 값 |
| --- | --- |
| 태스크 | `text-to-image` |
| 파라미터 | 9.4B |
| 라이선스 | other |
| 최초 등록일 | 2026-09-20 |
| 좋아요 | 35 |
| 다운로드 | 876 |

## 요약

🤖 ModelScope&nbsp;&nbsp;| &nbsp;&nbsp;🤗 HuggingFace&nbsp;&nbsp;| &nbsp;&nbsp;📑 Blog&nbsp;&nbsp;| &nbsp;&nbsp;🖥️ Demo&nbsp;&nbsp;| &nbsp;&nbsp;🫨 Discord

We are excited to open-source **Qwen-Image-2.1**, a unified text-to-image generation and image editing model in the Qwen family. With just **7B parameters in its visual generation component** (32 Single-Stream DiT layers), Qwen-Image-2.1 balances generation quality, inference efficiency, and versatility.

Four key improvements define this release:

## 라이선스

other — 상업 이용 제한 또는 확인 필요

## 관련 모델

- [Qwen Drive 1.0 4B](../Qwen__Qwen-Drive-1.0-4B/)
- [Qwen Image 2.1](../Qwen__Qwen-Image-2.1/)
- [Qwen Image 2.1 PE I2I](../Qwen__Qwen-Image-2.1-PE-I2I/)
