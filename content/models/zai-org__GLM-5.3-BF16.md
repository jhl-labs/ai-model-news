---
model_id: "zai-org/GLM-5.3-BF16"
title: "GLM 5.3 BF16"
org: "zai-org"
task: "text-generation"
license: "other"
params: "753.3B"
likes: 34
downloads: 5911
discovered_at: "2026-09-05"
created_at: "2026-08-25"
hf_url: "https://huggingface.co/zai-org/GLM-5.3-BF16"
tags: ["transformers", "safetensors", "glm_moe_dsa", "text-generation", "conversational", "en", "zh", "arxiv:2602.15763", "license:other", "endpoints_compatible", "region:us"]
reason: "major-org"
---

## 왜 주목받는가

주요 기관이 최근 30일 안에 공개한 신작입니다. 이 기준에 해당해 선정했습니다. 좋아요 34개, 다운로드 5,911회(수집 시점 2026-09-05). 이전 모델 zai-org/GLM-5.3(파라미터 753.3B, 다운로드 303,534) 대비 이번 모델은 파라미터 동일(753.3B), 다운로드 98.1% 감소, 라이선스 동일(other).

## 핵심 스펙

| 항목 | 값 |
| --- | --- |
| 태스크 | `text-generation` |
| 파라미터 | 753.3B |
| 라이선스 | other |
| 최초 등록일 | 2026-08-25 |
| 좋아요 | 34 |
| 다운로드 | 5,911 |

## 요약

GLM-5.3 uses the same base model as GLM-5.2 — every gain comes from post-training. Compared with GLM-5.2, it is much better at complex coding and long-horizon tasks:

+ Stronger Coding: GLM-5.3 is the most capable open-weights model for coding, with a 50% improvement over GLM-5.2 on our in-house Z.ai Code Bench. It also achieve open-source SOTA on public benchmarks including Terminal Bench 3.0 and Agents' Last Exam. + Emergent Cyber Capability: As we scaled post-training, cyber capability developed faster than we expected. GLM-5.3 is state of the art on CyberGym for vulnerability discovery, an…

## 라이선스

other — 상업 이용 제한 또는 확인 필요

## 관련 모델

- [GLM 5.3](../zai-org__GLM-5.3/)
- [GLM 5.3 Flash](../zai-org__GLM-5.3-Flash/)
- [GLM 5.3 Flash BF16](../zai-org__GLM-5.3-Flash-BF16/)
- [K2 Horizon MoVA 36B A4B](../IFM__K2-Horizon-MoVA-36B-A4B/)
- [Qwen3.8 27B OBLITERATED](../OBLITERATUS__Qwen3.8-27B-OBLITERATED/)
- [Qwen3.8 2.4T A95B](../Qwen__Qwen3.8-2.4T-A95B/)
