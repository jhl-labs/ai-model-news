---
model_id: "ibm-granite/granite-4.2-8b"
title: "granite 4.2 8b"
org: "ibm-granite"
task: "text-generation"
license: "apache-2.0"
params: "8.8B"
likes: 62
downloads: 14841
discovered_at: "2026-09-05"
created_at: "2026-08-07"
hf_url: "https://huggingface.co/ibm-granite/granite-4.2-8b"
tags: ["transformers", "safetensors", "granite", "text-generation", "granite-4.2", "reasoning", "thinking", "tool-calling", "ibm", "conversational", "en", "de", "es", "fr", "ja", "pt", "ar", "cs", "it", "ko", "nl", "zh", "base_model:ibm-granite/granite-4.1-8b-base", "base_model:finetune:ibm-granite/granite-4.1-8b-base", "license:apache-2.0", "eval-results", "endpoints_compatible", "region:us"]
reason: "major-org"
---

## 왜 주목받는가

주요 기관이 최근 30일 안에 공개한 신작입니다. 이 기준에 해당해 선정했습니다. 좋아요 62개, 다운로드 14,841회(수집 시점 2026-09-05). 이전 모델 ibm-granite/granite-4.2-3b(파라미터 3.7B, 다운로드 16,721) 대비 이번 모델은 파라미터 137.8% 증가, 다운로드 11.2% 감소, 라이선스 동일(apache-2.0).

## 핵심 스펙

| 항목 | 값 |
| --- | --- |
| 태스크 | `text-generation` |
| 파라미터 | 8.8B |
| 라이선스 | apache-2.0 |
| 최초 등록일 | 2026-08-07 |
| 좋아요 | 62 |
| 다운로드 | 14,841 |

## 요약

Granite is a family of open-source large language models developed by IBM, designed for enterprise and research use. Granite models are built to be versatile, safe, and efficient — covering a range of sizes and capabilities from compact edge-deployable models to large-scale reasoning systems. All Granite models are released under the Apache 2.0 license, enabling unrestricted commercial and academic use.

The Granite 4.2 generation introduces native reasoning (thinking) capabilities, allowing models to perform step-by-step chain-of-thought reasoning before producing final answers. This signific…

## 라이선스

apache-2.0 — 상업 이용 가능

## 관련 모델

- [granite 4.2 30b](../ibm-granite__granite-4.2-30b/)
- [granite 4.2 3b](../ibm-granite__granite-4.2-3b/)
- [K2 Horizon MoVA 36B A4B](../IFM__K2-Horizon-MoVA-36B-A4B/)
- [Qwen3.8 27B OBLITERATED](../OBLITERATUS__Qwen3.8-27B-OBLITERATED/)
- [Qwen3.8 2.4T A95B](../Qwen__Qwen3.8-2.4T-A95B/)
