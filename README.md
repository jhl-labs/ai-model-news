...
모든 경로의 공통 조건(신규성 게이트): **최근 60일 내 최초 공개(`createdAt`) 또는 14일 내 의미 있는 갱신(`lastModified`)**. 목록 API는 `full=true`로 요청해 갱신일을 함께 가져옵니다. 이 조건을 만족하지 못하면 트렌딩·급상승·주요 기관 여부와 상관없이 발행하지 않습니다. 고전 모델(gpt2, bert-base-uncased 등)은 신규성이 없어 제외됩니다.
